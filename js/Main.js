...
const App = {
  ...
  initFormElements() {
    ...
    searchContainer.innerHTML = '<div class="text-center py-2"><i class="fas fa-spinner fa-spin text-blue-500"></i></div>';
    ...
    try {
      ...
      const searchButton = Button.render({
        ...
        onClick: () => {
          ...
          const municipalityContent = document.getElementById('municipality-content');
          ...
          if (select && select.value && select.value !== select.options[0].value) {
            ...
            if (municipalityContent) municipalityContent.classList.remove('hidden');
            ...
            if (municipalityContent) municipalityContent.classList.add('hidden');
      ...
      searchContainer.innerHTML = '<p class="text-red-500 text-center">Erro ao carregar formulário</p>';
    }
  }, 
  ...
  async handleSearch() {
    ...
    if (statsContainer) {
      statsContainer.innerHTML = '<div class="text-center py-4"><i class="fas fa-spinner fa-spin text-blue-500 text-2xl"></i></div>';
    }
    
    // Limpar o container de tab
    const tabContainer = document.querySelector('.tab-content');
    if (tabContainer) {
      tabContainer.innerHTML = '<div class="text-center py-4"><i class="fas fa-spinner fa-spin text-blue-500 text-2xl"></i></div>';
    }
  ...
  renderTabContent(data = null) {
    ...
    if (!data) {
      ...
      if (this.state.cachedData[this.state.selectedMunicipality]) {
        this.renderTabContent(this.state.cachedData[this.state.selectedMunicipality]);
        return;
      }
      ...
      getMunicipality(this.state.selectedMunicipality)
        .then(municipalityData => {
          // Atualizar cache
          this.state.cachedData[this.state.selectedMunicipality] = municipalityData;
    ...
    try {
      ...
      if (data.repasses) {
        ...
        try {
          ...
          this.renderErrorMessage(tableContainer, 'Erro ao renderizar tabela de repasses');
        }
      }
      ...
      if (data.indices?.length > 0) {
        ...
        try {
          ...
          this.renderErrorMessage(positionContainer, 'Erro ao renderizar tabela de índices');
        }
        ...
        if (data.execucao?.length > 0) {
          ...
          try {
            ...
            this.renderErrorMessage(executionContainer, 'Erro ao renderizar tabela de execução');
          }
        }
      ...
      this.renderErrorMessage(container, `Erro ao renderizar conteúdo: ${error.message}`);
    }
  },
  ...
  renderVafChartSection(container, data) {
    ...
    import('../components/layout/ChartComponent.js')
      ...
      .catch(error => {
        ...
        this.renderErrorMessage(vafChartContainer, `Erro ao carregar gráfico: ${error.message}`);
      });
  },
  ...
  renderGraphicsTab(data) {
    ...
    try {
      ...
      import('../components/layout/ChartComponent.js')
        .then(module => {
          ...
          if (data.indicadores_serie?.length > 0) {
            ...
            new ChartComponent({
              ...
              type: 'line',
              data: ChartFormatter.prepareIndicadoresChartData(data.indicadores_serie),
              options: {
                tension: 0.4,
                pointRadius: 3,
                pointHoverRadius: 5
          ...
          if (data.repasses_serie?.length > 0) {
            ...
            new ChartComponent({
              ...
              type: 'line',
              data: ChartFormatter.prepareRepassesChartData(data.repasses_serie),
              options: {
                tension: 0.4,
                pointRadius: 3,
                pointHoverRadius: 5
        ...
        .catch(error => {
          console.error("Erro ao carregar componente de gráfico:", error);
          this.renderErrorMessage(container, `Erro ao carregar componente de gráfico: ${error.message}`);
        });
    } catch (error) {
      console.error("Erro ao renderizar tab de gráficos:", error);
      this.renderErrorMessage(container, `Erro ao renderizar gráficos: ${error.message}`);
    }
  },
