...
class Button {
  ...
  static render(config) {
    ...
    const styles = {
      ...
      outline:
        "bg-transparent border border-blue-medium text-blue-medium hover:bg-blue-medium hover:text-white",
    };

    const buttonStyle = styles[config.type || "primary"];
    const iconHtml = config.icon
      ? `<i class="fa-solid fa-${config.icon} ml-1 text-sm"></i>`
      : "";

    const button = document.createElement("button");
    button.id = config.id || "";
    button.className = `${buttonStyle} font-medium px-4 py-2 rounded-lg flex-shrink-0 transition-colors duration-200 flex items-center justify-center ${
      config.className || ""
    }`;
