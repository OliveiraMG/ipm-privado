...
class HeaderDropdown {
   ...
   */
  constructor(config) {
    this.targetElement = typeof config.targetElement === 'string' 
  ...
  show() {
    ...
    this.dropdownElement.className = 'absolute bg-white rounded-lg shadow-lg z-50 py-2 mt-1 border border-gray-200';
    ...
    // Adicionar eventos
    this.dropdownElement.addEventListener('mouseenter', this.handleDropdownMouseEnter.bind(this));
    this.dropdownElement.addEventListener('mouseleave', this.handleMouseLeave.bind(this));
  ...
  renderCategorizedMenu() {
    ...
    this.categories.forEach((category, index) => {
      ...
      title.className = 'text-blue-dark font-medium text-base mb-4 border-b pb-2';
  ...
  createMenuItem(item) {
    ...
    link.href = item.url || '#';
    link.className = 'flex items-center px-3 py-1.5 text-gray-700 hover:bg-blue-50 hover:text-blue-dark rounded-md text-sm';
    ...
    if (item.badge) {
      ...
      badge.className = 'ml-2 text-xs px-1.5 py-0.5 bg-blue-light text-white rounded-full';
  ...
  setPosition() {
    ...
    switch (this.position) {
      case 'bottom-center': {
        ...
        const finalLeft = Math.max(10, Math.min(leftPos, windowWidth - dropdownWidth - 10));
      ...
      case 'bottom-right':
        this.dropdownElement.style.top = `${rect.bottom + scrollTop}px`;
        this.dropdownElement.style.right = `${window.innerWidth - rect.right - scrollLeft}px`;
        break;
      default:
        this.dropdownElement.style.top = `${rect.bottom + scrollTop}px`;
        this.dropdownElement.style.left = `${rect.left + scrollLeft}px`;
