...
class ImportPage {
  ...
  renderImportForm() {
    ...
    card.className = 'bg-white rounded-3xl shadow-lg mt-6 mb-10 overflow-hidden';

    // Cabeçalho do card
    const header = document.createElement('div');
    header.className = 'bg-blue-dark text-white py-4 px-6 text-center';
    header.innerHTML = '<h2 class="text-xl font-medium text-white">Importar Arquivos</h2>';
    ...
    const documentTypes = [
      ...
      { value: 'ACGPT109', text: 'ACGPT109 - NFE-Eletrônica: Relatório de Notas Fiscais de entrada eletrônicas' },
      { value: 'ACGPT415', text: 'ACGPT415 - NFE-EFD: Relatório de Notas Fiscais de entrada eletrônicas EFD' },
      { value: 'ACGPT420', text: 'ACGPT420 - EFD – OMISSO' },
      { value: 'ACGPT425', text: 'ACGPT425 - EFD – SEM MOVIMENTO' },
      { value: 'ACGPT430', text: 'ACGPT430 - EFD-ESCRITURAÇÃO FISCAL DIGITAL – registros por CFOPs' },
      { value: 'ACGPR812', text: 'ACGPR812 - Notas Fiscais de Produtor e Avulsa (NFPA) – eletrônicas' },
      { value: 'PGDAS', text: 'PGDAS - Receita dos contribuintes Simples Nacional' },
      { value: 'DHRPR555', text: 'DHRPR555 - Valores adicionados por contribuinte (positivos)' },
      { value: 'DHRPR557', text: 'DHRPR557 - Valores adicionados por contribuinte (negativos e nulos)' },
      ...
      { value: 'DHRPR098', text: 'DHRPR098 - GIA-ICMS Sem movimento (entradas e saídas zeradas)' },
      { value: 'DHRPR278', text: 'DHRPR278 - GIA-ICMS Omissos' },
      { value: 'DHRPR296', text: 'DHRPR296 - GIA-ICMS – Registros por CFOPs (entradas e saídas)' },
      { value: 'COP3', text: 'COP3 - GIA-ICMS – anexo I' },
      { value: 'ACGPT400-410', text: 'ACGPT400-410 - Reg. 1400 EFD: registros valores prestados por município' },
      { value: 'ACYPR535', text: 'ACYPR535 - Relação dos Índices Apurados' },
      { value: 'ACYPR540', text: 'ACYPR540 - Relação das Variações dos Índices' }
    ];
  ...
  setupEventListeners() {
    ...
    setTimeout(() => {
      ...
      if (fileTypeSelect) {
        fileTypeSelect.addEventListener('change', (e) => this.handleFileTypeChange(e));
      }
    }, 100);
  }
  ...
  handleFileTypeChange(event) {
    ...
    if (selectedType === 'avulso') {
      ...
      descriptionInput.className = 'border border-gray-300 rounded-lg w-full px-4 py-2 text-gray-700';
  ...
  handleSubmit() {
    ...
    if (!fileType || !remessa || !apuracao || !documentType || !fileInput.files[0]) {
    ...
    if (fileType === 'avulso') {
      ...
      const fileTypeExt = document.getElementById('file-type-ext-select')?.value;
