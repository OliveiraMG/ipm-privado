/**
 * IndiceAnteriorAtual.js - Página de Lotes Gerados
 */
import ModalComponent from "../../../components/common/ModalComponent.js";
import { Header } from "../../../components/layout/Header.js";

class IndiceAnteriorAtual {
  constructor() {
    this.tableData = [];
    this.modal = null;
    this.currentPage = 1;
    this.itemsPerPage = 10; // Adjusted for smaller screens, can be changed
    this.initialize();
  }

  initialize() {
    console.log("Inicializando IndiceAnteriorAtual...");
    Header.initialize();
    ...
    if (breadcrumbContainer) {
      breadcrumbContainer.innerHTML = `
        <div class="pt-1 pb-1 pl-4 pr-4 md:pl-20 md:pr-16">
          <div class="text-gray-500 text-base leading-tight">Gerenciar | Estatisticas | Indice Anterior e Atual</div>
          <div class="text-blue-dark font-semibold text-2xl md:text-4xl leading-tight mt-0">Indice Anterior e Atual</div>
        </div>
      `;
    }
  }
  ...
  renderContent() {
    ...
    card.className =
      "bg-white rounded-2xl shadow-[6px_6px_12px_rgba(0,0,0,0.25)] mx-4 my-8 md:mx-24 md:my-20 p-6 md:p-12"; // Adjusted margins and padding for responsiveness

    const header = document.createElement("div");
    header.className =
      "flex justify-between items-start md:items-center mb-4 md:mb-6 flex-col md:flex-row gap-2"; // Added gap for better layout
    ...
    content.className = "text-gray-700 text-base mb-4 md:mb-6 overflow-x-auto"; // Added overflow-x-auto for table overflow on small screens
    ...
    <table class="w-full table-auto border-collapse rounded-xl overflow-hidden">
      <thead class="bg-[#23424A] text-white">
        <tr>
          ...
          <th class="px-2 py-1 md:px-4 md:py-2 text-left">IMP Anterior</th>
          <th class="px-2 py-1 md:px-4 md:py-2 text-left">IMP Atual</th>
          <th class="px-2 py-1 md:px-4 md:py-2 text-left">Porcentagem</th>
        </tr>
      </thead>
      <tbody>
        ${data
          .map(
            ...
            <tr class="border-b hover:bg-gray-50">
              <td class="px-2 py-1 md:px-4 md:py-2 border-2">${item.apuracao}</td>
              <td class="px-2 py-1 md:px-4 md:py-2 border-2">${item.municipio}</td>
              <td class="px-2 py-1 md:px-4 md:py-2 border-2">${item.impAnterior}</td>
              <td class="px-2 py-1 md:px-4 md:py-2 border-2">${item.impAtual}</td>
              <td class="px-2 py-1 md:px-4 md:py-2 border-2">${item.porcentagem}</td>
            </tr>
          `
          )
          .join("")}
...
`;
  ...
  getMockData() {
    ...
    const items = [];
    for (let i = 1; i <= 42; i++) {
      const impAnterior = parseFloat((Math.random() * 5).toFixed(2));
      const impAtual = parseFloat(
        (impAnterior + Math.random() * 2 - 1).toFixed(2)
      );

      items.push({
        apuracao: `2024-${(i % 12) + 1 < 10 ? "0" : ""}${(i % 12) + 1}`,
        municipio: municipios[i % municipios.length],
        impAnterior,
        impAtual,
        porcentagem: ((impAtual - impAnterior) / impAnterior).toFixed(2),
      });
    }
  ...
  static initialize() {
    return new IndiceAnteriorAtual();
  }
}

document.addEventListener("DOMContentLoaded", () => {
  IndiceAnteriorAtual.initialize();
});

export default IndiceAnteriorAtual;
