/**
 * VaCatEconomicaPage.js - Página de VA, C
 */
import ModalComponent from "../../../components/common/ModalComponent.js";
import { Header } from "../../../components/layout/Header.js";

class VaCatEconomicaPage {
  constructor() {
    this.tableData = [];
    this.modal = null;
    this.currentPage = 1;
  ...
  initialize() {
    console.log("Inicializando VaCatEconomicaPage...");
    Header.initialize();
    ...
    if (breadcrumbContainer) {
      breadcrumbContainer.innerHTML = `
        <div class="pt-1 pb-1 pl-20 pr-16">
          <div class="text-gray-500 text-base leading-tight">Gerenciar | Estatisticas | VA por Cat Econômica e Outros</div>
          <div class="text-blue-dark font-semibold text-4xl leading-tight mt-0">VA por Cat Econômica e Outros</div>
        </div>
      `;
    }
  }
    ...
    <table class="w-full table-auto border-collapse rounded-xl overflow-hidden">
      <thead class="bg-[#23424A] text-white ">
        <tr>
          ...
          <th class="px-4 py-2 text-left">Industrial</th>
          <th class="px-4 py-2 text-left">Rural</th>
          <th class="px-4 py-2 text-left">Prest. Serv.</th>
          <th class="px-4 py-2 text-left">DAR</th>
          <th class="px-4 py-2 text-left">NAI</th>
          <th class="px-4 py-2 text-left">Crédito</th>
          <th class="px-4 py-2 text-left">Débito</th>
        ...
        ${data
          .map(
            ...
            <tr class="border-b hover:bg-gray-50">
              ...
              <td class="px-4 py-2 border-2">${item.industrial}</td>
              <td class="px-4 py-2 border-2">${item.rural}</td>
              <td class="px-4 py-2 border-2">${item.prestacaoServico}</td>
              <td class="px-4 py-2 border-2">${item.dar}</td>
              <td class="px-4 py-2 border-2">${item.nai}</td>
              <td class="px-4 py-2 border-2">${item.credito}</td>
              <td class="px-4 py-2 border-2">${item.debito}</td>
              <td class="px-4 py-2 border-2">${item.total}</td>
            </tr>
          `
          )
          .join("")}
  ...
  getMockData() {
    ...
    for (let i = 1; i <= 42; i++) {
      items.push({
        ...
        industrial: Math.floor(Math.random() * 1000000),
        rural: Math.floor(Math.random() * 1000000),
        prestacaoServico: Math.floor(Math.random() * 1000000),
        dar: Math.floor(Math.random() * 1000000),
        nai: Math.floor(Math.random() * 1000000),
        credito: Math.floor(Math.random() * 1000000),
        debito: Math.floor(Math.random() * 1000000),
  ...
  static initialize() {
    return new VaCatEconomicaPage();
  }
}

document.addEventListener("DOMContentLoaded", () => {
  VaCatEconomicaPage.initialize();
});

export default VaCatEconomicaPage;
