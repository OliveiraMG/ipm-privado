...
class EditPortariaComponent {
  ...
  render() {
    ...
    const [numero, ano] = this.portariaData.numeroAno ? this.portariaData.numeroAno.split('/') : ['', ''];
      ...
      <div class="grid grid-cols-5 gap-4">
        <div class="col-span-1">
          ...
          <select id="emVigor" name="emVigor"
            ...
            <option value="Sim" ${this.portariaData.vigente === 'Sim' ? 'selected' : ''}>Sim</option>
            <option value="Não" ${this.portariaData.vigente === 'Não' ? 'selected' : ''}>Não</option>
          </select>
        </div>
        <div class="col-span-1">
        ...
        <div class="col-span-1">
          ...
          <input type="text" id="dataAssinatura" name="dataAssinatura" value="${this.portariaData.assinatura || ''}" placeholder="DD/MM/YYYY"
                 class="mt-1 block w-full border border-gray-300 rounded-md p-2 text-sm focus:ring-blue-light focus:border-blue-light">
        </div>
        <div class="col-span-1">
          <label for="dataPublicacao" class="block text-sm font-medium text-gray-700">Data Publicação</label>
          <input type="text" id="dataPublicacao" name="dataPublicacao" value="${this.portariaData.publicacao || ''}" placeholder="DD/MM/YYYY"
                 class="mt-1 block w-full border border-gray-300 rounded-md p-2 text-sm focus:ring-blue-light focus:border-blue-light">
        </div>
      </div>
      <div class="mt-4">
        <label for="portaria" class="block text-sm font-medium text-gray-700">Portaria</label>
        <input type="text" id="portaria" name="portaria" value="${this.portariaData.portaria || ''}" readonly
               class="mt-1 block w-full border border-gray-300 rounded-md p-2 text-sm bg-gray-100">
      </div>
      <div class="mt-4">
        <label for="descricao" class="block text-sm font-medium text-gray-700">Descrição</label>
        <input type="text" id="descricao" name="descricao" value="${this.portariaData.descricao || ''}"
               class="mt-1 block w-full border border-gray-300 rounded-md p-2 text-sm focus:ring-blue-light focus:border-blue-light">
      </div>
      <div class="mt-4 grid grid-cols-2 gap-4">
        <div class="col-span-1">
          ...
          <select id="cfopSelect" name="cfopSelect"
                  class="mt-1 block w-full border border-gray-300 rounded-md p-2 text-sm focus:ring-blue-light focus:border-blue-light">
            <option value="" ${!this.portariaData.cfopSelect ? 'selected' : ''}>Selecione</option>
            <option value="repercute" ${this.portariaData.cfopSelect === 'repercute' ? 'selected' : ''}>repercute no cálculo do Demonstrativo (Relatório)</option>
          </select>
        </div>
      </div>
      <div class="mt-4 grid grid-cols-2 gap-4">
        <div class="col-span-1">
          ...
          <textarea id="ementa" name="ementa" rows="4"
                    class="mt-1 block w-full border border-gray-300 rounded-md p-2 text-sm focus:ring-blue-light focus:border-blue-light">${this.portariaData.ementa || ''}</textarea>
        </div>
        <div class="col-span-1">
          <label for="cfop" class="block text-sm font-medium text-gray-700">Cfop</label>
          <textarea id="cfop" name="cfop" rows="4"
                    class="mt-1 block w-full border border-gray-300 rounded-md p-2 text-sm focus:ring-blue-light focus:border-blue-light">${this.portariaData.cfop || ''}</textarea>
