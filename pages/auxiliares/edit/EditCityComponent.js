...
class EditCityComponent {
    ...
    form.innerHTML = `
      <div class="grid grid-cols-2 gap-4">
        <div class="col-span-1">
          ...
          <input type="text" id="codigo" name="codigo" value="${this.cityData.codigo || ''}" readonly
                 class="mt-1 block w-full border border-gray-300 rounded-md p-2 text-sm bg-gray-100">
        </div>
        <div class="col-span-1">
          <label for="descricao" class="block text-sm font-medium text-gray-700">Descrição:</label>
          <input type="text" id="descricao" name="descricao" value="${this.cityData.cidade || ''}" required
        ...
        <div class="col-span-1">
          ...
          <select id="estado" name="estado" required
                  class="mt-1 block w-full border border-gray-300 rounded-md p-2 text-sm focus:ring-blue-light focus:border-blue-light">
            <option value="Mato Grosso" ${this.cityData.estado === 'Mato Grosso' ? 'selected' : ''}>Mato Grosso</option>
            <option value="Rondônia" ${this.cityData.estado === 'Rondônia' ? 'selected' : ''}>Rondônia</option>
          </select>
        </div>
        <div class="col-span-1">
          ...
          <select id="ativo" name="ativo" required
                  class="mt-1 block w-full border border-gray-300 rounded-md p-2 text-sm focus:ring-blue-light focus:border-blue-light">
            <option value="true" ${this.cityData.ativo === 'Sim' ? 'selected' : ''}>Sim</option>
            <option value="false" ${this.cityData.ativo === 'Não' ? 'selected' : ''}>Não</option>
          </select>
        </div>
        <div class="col-span-2 flex items-center space-x-4">
          <div class="flex-1">
            ...
            <div class="flex items-center mt-1">
              <span id="brasao-file-name" class="text-gray-500 text-sm mr-4">${this.cityData.brasao ? 'Atual:' : 'Escolher Brasão'}</span>
    ...
    if (chooseBrasaoBtn && brasaoInput) {
      ...
      brasaoInput.addEventListener('change', () => {
        if (brasaoInput.files && brasaoInput.files.length > 0) {
          ...
          brasaoFileName.textContent = this.cityData.brasao ? 'Atual:' : 'Escolher Brasão';
  ...
  submitForm() {
    ...
    if (descricaoInput.checkValidity() && estadoInput.checkValidity() && ativoInput.checkValidity()) {
      const updatedData = {
        id: this.cityData.id,
        ...
        brasao: brasaoInput.files && brasaoInput.files.length > 0 ? URL.createObjectURL(brasaoInput.files[0]) : this.cityData.brasao
      };
