...
class UsuariosPage {
  ...
  setupButtonEvents() {
    ...
    if (removedBtn) {
      removedBtn.addEventListener("click", () => {
        toast.info(
          "Funcionalidade de visualizar removidos ainda não implementada."
        );
      });
    }
  }
    ...
    const sincronizarPerfisComponent = new SincronizarPerfisUsuarioComponent({
      ...
      onSync: (data) => {
        this.tableData = this.tableData.map((item) =>
          item.id === userId
            ? { ...item, perfil: data.profile, tipoUsuario: data.profile }
            : item
        );
        ...
        this.modal.close();
        toast.success(
          `Perfil de ${usuarioData.nome} sincronizado com sucesso!`
        );
      },
      onBack: () => {
        this.modal.close();
  ...
  openSincronizarPermissoesModal(userId) {
    ...
    const sincronizarPermissoesComponent =
      new SincronizarPermissoesUsuarioComponent({
        userName: usuarioData.nome,
        onSync: (data) => {
          console.log(
            `Permissões sincronizadas para ${usuarioData.nome}:`,
            data.permissions
          );
          this.modal.close();
          toast.success(
            `Permissões de ${usuarioData.nome} sincronizadas com sucesso!`
          );
        },
        onBack: () => {
          this.modal.close();
          ...
          <tr class="${rowClass}">
            <td class="text-center py-2 px-4">
              <div class="flex space-x-2 justify-center">
                ...
                <button class="profiles-btn bg-blue-dark text-white text-xs rounded px-2 py-1 hover:bg-blue-medium" data-id="${
                  row.id || ""
                }" style="display: inline-block !important;">
                ...
                <button class="delete-btn bg-red-500 text-white text-xs rounded px-2 py-1 hover:bg-red-600" data-id="${
                  row.id || ""
                }" style="display: inline-block !important;">
  ...
  setupTableButtonEvents() {
    ...
    deleteButtons.forEach((button) => {
      button.addEventListener("click", () => {
        const userId = parseInt(button.getAttribute("data-id"));
        toast.info(
          `Funcionalidade de remoção do usuário ${userId} ainda não implementada.`
        );
      });
    });
  }
