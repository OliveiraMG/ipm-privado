/**
 * NfAvulsasPage.js - Página de Listagem de Notas Fiscais Avulsas
 */
...
class NfAvulsasPage {
  ...
  initialize() {
    console.log("Inicializando NfAvulsasPage...");
    Header.initialize();
    ...
    if (breadcrumbContainer) {
      breadcrumbContainer.innerHTML = `
        <div class="pt-1 pb-1 pl-4 pr-4 md:pl-20 md:pr-16">
          <div class="text-gray-500 text-base leading-tight">Cadastro | Contribuinte | NF-e Avulsas</div>
          <div class="text-blue-dark font-semibold text-2xl md:text-4xl leading-tight mt-0">NF-e Avulsas</div>
        </div>
      `;
    }
  }
...
`;
            ...
            <tr class="border-b hover:bg-gray-100 border">
              <td class="px-2 py-1 md:px-4 md:py-2 text-sm whitespace-nowrap">
                <div class="flex flex-col sm:flex-row gap-2">
                  ...
                  <button
                    class="bg-[#23424A] hover:opacity-60 transition-all duration-300 ease-in-out text-white px-2 py-1 rounded text-xs md:text-sm flex items-center gap-1 print-importado-btn w-full sm:w-auto"
                    data-index="${index}"
                  >
  ...
  getMockData() {
    const items = [];
    for (let i = 1; i <= 42; i++) {
      const valorMocked = parseFloat((Math.random() * 5).toFixed(2));
  ...
  static initialize() {
    return new NfAvulsasPage();
  }
}

document.addEventListener("DOMContentLoaded", () => {
  NfAvulsasPage.initialize();
});

export default NfAvulsasPage;
