...
const MOCK_DATA = {
  "Pontes e Lacerda": {
    ...
    vaf: [
      { ano: 2020, vaf_anterior: 1109862.9197, vaf_atual: 850000.0 },
      { ano: 2021, vaf_anterior: 1893394.68748, vaf_atual: 1300000.0 },
      { ano: 2022, vaf_anterior: 2130774.16022, vaf_atual: 2800000.0 },
      { ano: 2023, vaf_anterior: 2475495.06834, vaf_atual: 2000000.0 },
      { ano: 2024, vaf_anterior: 2650000.0, vaf_atual: 2300000.0 },
    ],
  },
  Cuiabá: {
    contribuintes: {
    ...
    vaf: [
      ...
      { ano: 2024, vaf_anterior: 5800000.0, vaf_atual: 5600000.0 },
    ],
  },
  "Várzea Grande": {
    contribuintes: {
...
export const getMunicipality = async (municipalityName) => {
      ...
      setTimeout(() => {
        ...
        if (data) {
          ...
          const error = new Error(
            `Município não encontrado: ${municipalityName}`
          );
          console.error(error);
          reject(error);
        }
  ...
  try {
    const url = `${API_BASE_URL}/municipios/${encodeURIComponent(
      municipalityName
    )}`;
    ...
    if (!response.ok) {
      const error = new Error(
        `Erro ao buscar município: ${response.statusText}`
      );
      console.error(error);
      throw error;
    }
    ...
    if (!data || Object.keys(data).length === 0) {
      const error = new Error(
        `Não foram encontrados dados para o município: ${municipalityName}`
      );
      console.error(error);
      throw error;
    }
